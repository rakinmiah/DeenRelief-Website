...
import { PALESTINE_UPSELL } from "./upsell";
...
export default function DonationForm() {
  ...
  /**
   * Outcome label: exact tier match shows its outcome verbatim; custom
   * amounts between tiers floor to the highest tier at or below the value
   * and show that tier's outcome directly (no "approximately" hedge — reads
   * as a specific, confident impact statement).
   */
  ...
  /**
   * Validation for the custom amount. Blocks sub-£5 donations (processing
   * fees eat most of it) — no upper cap so large donors aren't blocked.
   */
  ...
  const upsellChips = getUpsellChips(amountForUrl, PALESTINE_UPSELL);
  ...
  return (
    <div>
      ...
      {/* CTA — routes to /donate checkout with campaign + amount + frequency params */}
      {isAmountValid ? (
        <Button
          variant="primary"
          size="lg"
      ...
      {/* Accepted payment methods — reduces friction anxiety before click */}
      ...
      {/* Alternative giving routes — free conversion recovery for Zakat/Sadaqah seekers */}
