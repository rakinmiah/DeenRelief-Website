...
/**
 * Stripped-down donation picker used in the Final CTA section. Shows the
 * frequency toggle, four preset amount tiles, a custom amount input, and a
 * dynamic donate button — nothing else. Styled for a dark-green background,
 * fully centered. Shares donation data with the main DonationForm via a
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div
        ...
        className="flex items-center gap-1 mb-5 bg-white/10 backdrop-blur-sm rounded-full p-1 w-fit mx-auto"
      >
        <button
          type="button"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3">
        {amounts.map((amount) => (
          <button
            ...
            className={`relative py-3 px-3 rounded-xl text-center font-semibold transition-all duration-200 border-2 ${
              selectedAmount === amount.value
                ? "border-amber bg-cream text-charcoal"
                : "border-white/25 bg-white/5 text-white hover:border-white/50 hover:bg-white/10"
            }`}
          >
            {amount.label}
            {amount.default && (
              <span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] bg-amber text-charcoal px-1.5 py-px rounded-full font-bold">
                Popular
              </span>
            )}
          </button>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className={`w-full pl-8 pr-4 py-3 rounded-xl border-2 text-white placeholder:text-white/40 bg-white/5 backdrop-blur-sm text-center transition-colors duration-200 focus:outline-none ${
            isCustom
              ? "border-amber"
      ...
      {isAmountValid ? (
        ...
        <button
          ...
          disabled
          aria-disabled="true"
          className="inline-flex items-center justify-center px-8 py-4 rounded-full bg-white/10 text-white/40 font-semibold text-base cursor-not-allowed"
        >
