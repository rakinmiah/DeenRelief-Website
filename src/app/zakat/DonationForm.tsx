...
export default function DonationForm() {
  ...
  return (
    <div className="text-center">
      <span className="inline-block text-[11px] font-bold tracking-[0.1em] uppercase text-green mb-3">Pay Your Zakat</span>
      <h2 className="text-3xl sm:text-4xl font-heading font-bold text-charcoal leading-tight mb-3">Pay Your Zakat With Confidence</h2>
      <p className="text-grey text-base sm:text-[1.0625rem] leading-[1.7] mb-2">Takes under 2 minutes. Your £100 becomes £125 with Gift Aid.</p>
      ...
      <div className="mb-5">
        <div className="relative">
          ...
          <input type="number" inputMode="numeric" placeholder="Custom amount" value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setSelectedAmount(Number(e.target.value) || 0); }} onFocus={() => setSelectedAmount(0)} className={`w-full pl-8 pr-4 py-3.5 rounded-xl border-2 text-charcoal placeholder:text-grey/50 transition-colors duration-200 focus:outline-none ${customAmountError ? "border-red-500 bg-red-50" : isCustom ? "border-green bg-green-light" : "border-grey-light focus:border-green/40"}`} min={MIN_AMOUNT} aria-label="Enter a custom Zakat amount in pounds" aria-invalid={!!customAmountError} aria-describedby={customAmountError ? "custom-amount-error" : undefined} />
        </div>
      ...
      <p className="mt-3 pt-3 border-t border-grey-light/70 text-[12px] text-grey/70">
        Looking to give Sadaqah instead?{" "}
