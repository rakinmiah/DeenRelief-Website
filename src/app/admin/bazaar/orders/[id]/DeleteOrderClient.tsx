 ...
 *   - Expand → typed `DELETE` confirm → permanently nukes the row
 *
 * Two safety layers above the typed confirm:
 *   1. The expanded panel explains EXACTLY what will be deleted —
    ...
    <section className="bg-white border border-red-200 rounded-2xl p-6">
      ...
      {isHighRisk && (
        <div className="mb-4 rounded-xl bg-amber-light border border-amber/30 p-4 text-[13px] text-amber-dark leading-relaxed">
          <p className="font-semibold mb-1">This is a live, paid order.</p>
          <p>
            Deleting wipes the only record of a real customer
            transaction ({totalFormatted} to {customerEmail}). If you
            need to reverse the order, the right tool is{" "}
            <strong>Issue refund</strong>, not delete. Only continue
            if this row is genuinely test/accidental data that
            slipped into live mode.
          </p>
        </div>
      )}
