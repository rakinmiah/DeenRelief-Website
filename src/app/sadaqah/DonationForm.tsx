...
export default function DonationForm() {
  ...
  return (
    <div>
      <span className="inline-block text-[11px] font-bold tracking-[0.1em] uppercase text-green mb-3">Give Freely</span>
      <h2 className="text-3xl sm:text-4xl font-heading font-bold text-charcoal leading-tight mb-3">Every Penny Counts</h2>
      <p className="text-grey text-base sm:text-[1.0625rem] leading-[1.7] mb-1">No minimum, no calculation. Just generosity. Gift Aid adds 25%.</p>
      <p className="text-green text-xs font-semibold mb-6">Trusted by 3,200+ donors since 2013</p>

      <div role="group" aria-label="Donation frequency" className="flex items-center gap-1 mb-5 bg-grey-light rounded-full p-1 w-fit">
        <button type="button" onClick={() => handleFrequencyChange("one-time")} aria-pressed={frequency === "one-time"} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${frequency === "one-time" ? "bg-white text-charcoal shadow-sm" : "text-charcoal/50 hover:text-charcoal/70"}`}>One-time</button>
        <button type="button" onClick={() => handleFrequencyChange("monthly")} aria-pressed={frequency === "monthly"} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${frequency === "monthly" ? "bg-white text-charcoal shadow-sm" : "text-charcoal/50 hover:text-charcoal/70"}`}>Monthly</button>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
        {amounts.map((amount) => (
          <button key={amount.value} onClick={() => { setSelectedAmount(amount.value); setCustomAmount(""); }} className={`relative py-3 px-4 rounded-xl text-center font-semibold transition-all duration-200 border-2 ${selectedAmount === amount.value ? "border-green bg-green-light text-green" : "border-grey-light bg-white text-charcoal hover:border-green/40"}`}>
            {amount.label}
            {amount.default && (<span className="absolute -top-2 left-1/2 -translate-x-1/2 text-[8px] bg-green text-white px-1.5 py-px rounded-full">Popular</span>)}
          </button>
        ))}
      </div>
      ...
      <div className="mb-4">
        <div className="relative">
          ...
          <input type="number" inputMode="numeric" placeholder="Custom amount" value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setSelectedAmount(Number(e.target.value) || 0); }} onFocus={() => setSelectedAmount(0)} className={`w-full pl-8 pr-4 py-3 rounded-xl border-2 text-charcoal placeholder:text-grey/50 transition-colors duration-200 focus:outline-none ${customAmountError ? "border-red-500 bg-red-50" : isCustom ? "border-green bg-green-light" : "border-grey-light focus:border-green/40"}`} min={MIN_AMOUNT} aria-label="Enter a custom Sadaqah amount in pounds" aria-invalid={!!customAmountError} aria-describedby={customAmountError ? "custom-amount-error" : undefined} />
        </div>
        {customAmountError && (<p id="custom-amount-error" className="text-[12px] text-red-600 font-medium mt-1.5 ml-1" role="alert">{customAmountError}</p>)}
      </div>
      ...
      {isAmountValid ? (
        <Button variant="primary" size="lg" href={`/donate?campaign=sadaqah&amount=${amountForUrl}&frequency=${frequency}`} className="w-full sm:w-auto">
          {frequency === "monthly" ? `Give £${amountForUrl}/month` : `Give Sadaqah — £${amountForUrl.toLocaleString()}`}
        </Button>
      ) : (
        <button type="button" disabled aria-disabled="true" className="inline-flex w-full sm:w-auto items-center justify-center px-8 py-4 rounded-full bg-grey-light text-charcoal/40 font-semibold text-lg cursor-not-allowed">Enter £{MIN_AMOUNT} or more to continue</button>
      )}
      ...
      <p className="mt-3 pt-3 border-t border-grey-light/70 text-[12px] text-grey/70">
        Looking to pay Zakat instead?{" "}
        <Link href="/zakat" className="font-semibold text-green hover:text-green-dark underline underline-offset-2 decoration-green/30 hover:decoration-green">Pay Zakat</Link>
      </p>
    </div>
  );
}
