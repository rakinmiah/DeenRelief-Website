...
export default function DonationForm() {
  ...
  return (
    <div className="text-center">
      ...
      <h2 className="text-3xl sm:text-4xl font-heading font-bold text-charcoal leading-tight mb-3">Help Us Feed Our Community</h2>
      <p className="text-grey text-base sm:text-[1.0625rem] leading-[1.7] mb-2">Your donation funds weekly outreach on Brighton&apos;s streets. Gift Aid adds 25%.</p>
      <p className="text-green text-xs font-semibold mb-6">Trusted by 3,200+ donors since 2013</p>

      <div role="group" aria-label="Donation frequency" className="flex items-center justify-center gap-1 mb-6 bg-grey-light rounded-full p-1 w-fit mx-auto">
        <button type="button" onClick={() => handleFrequencyChange("one-time")} aria-pressed={frequency === "one-time"} className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 ${frequency === "one-time" ? "bg-white text-charcoal shadow-sm" : "text-charcoal/50 hover:text-charcoal/70"}`}>One-time</button>
        <button type="button" onClick={() => handleFrequencyChange("monthly")} aria-pressed={frequency === "monthly"} className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 ${frequency === "monthly" ? "bg-white text-charcoal shadow-sm" : "text-charcoal/50 hover:text-charcoal/70"}`}>Monthly</button>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
        {amounts.map((amount) => (
          <button key={amount.value} onClick={() => { setSelectedAmount(amount.value); setCustomAmount(""); }} className={`relative py-3.5 px-4 rounded-xl text-center font-semibold transition-all duration-200 border-2 ${selectedAmount === amount.value ? "border-green bg-green-light text-green" : "border-grey-light bg-white text-charcoal hover:border-green/40"}`}>
            {amount.label}
      ...
      <div className="mb-5">
        <div className="relative">
          ...
          <input type="number" inputMode="numeric" placeholder="Custom amount" value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setSelectedAmount(Number(e.target.value) || 0); }} onFocus={() => setSelectedAmount(0)} className={`w-full pl-8 pr-4 py-3.5 rounded-xl border-2 text-charcoal placeholder:text-grey/50 transition-colors duration-200 focus:outline-none ${customAmountError ? "border-red-500 bg-red-50" : isCustom ? "border-green bg-green-light" : "border-grey-light focus:border-green/40"}`} min={MIN_AMOUNT} aria-label="Enter a custom donation amount in pounds" aria-invalid={!!customAmountError} aria-describedby={customAmountError ? "custom-amount-error" : undefined} />
        </div>
        {customAmountError && (<p id="custom-amount-error" className="text-[12px] text-red-600 font-medium mt-1.5" role="alert">{customAmountError}</p>)}
      </div>
      ...
      {isAmountValid ? (
        <Button variant="primary" size="lg" href={`/donate?campaign=uk-homeless&amount=${amountForUrl}&frequency=${frequency}`} className="w-full justify-center">
          Donate £{amountForUrl.toLocaleString()}{frequency === "monthly" ? "/month" : ""} Now
        </Button>
      ) : (
      ...
      <p className="mt-3 pt-3 border-t border-grey-light/70 text-[12px] text-grey/70">
        ...
        <Link href="/sadaqah" className="font-semibold text-green hover:text-green-dark underline underline-offset-2 decoration-green/30 hover:decoration-green">Give Sadaqah</Link>
      </p>
    </div>
  );
}
